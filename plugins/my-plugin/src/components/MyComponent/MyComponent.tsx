...
async function runPipeline(pipelineId : TableData, backendBaseUrl : Object ): Promise<void>
  {
    await fetch(`${await backendBaseUrl}/harness/gateway/pipeline/api/pipeline/execute/${pipelineId.pipelineId}?routingId=dh-iBL35SqqpuqJF0yDjpQ&accountIdentifier=dh-iBL35SqqpuqJF0yDjpQ&projectIdentifier=CIQuickstart&orgIdentifier=default&moduleType=ci`, {
      "headers": {
...
function MyComponent() {
  ...
  const columns: TableColumn[] = [
    ...
    },
    {
      ...
      render: (row : Partial<TableData>) => (
        <div>
          <IconButton aria-label="replay" onClick={() => runPipeline(Object(row), backendBaseUrl)}>
            <ReplayIcon/>
          </IconButton>
        </div>
