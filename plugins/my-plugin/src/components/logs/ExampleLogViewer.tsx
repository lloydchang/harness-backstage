...
function ExampleLogViewer(row : any){
  ...
  const orgid = entity?.metadata?.annotations?.['orgIdentifier'];
  const projectid = entity?.metadata?.annotations?.['projectIdentifier'];


  const response = await fetch(`${await backendBaseUrl}/harness/gateway/log-service/blob?accountID=${accid}&X-Harness-Token=&key=accountId:${accid}/orgId:${orgid}/projectId:${projectid}/pipelineId:${row.row.pipelineIdentifier}/runSequence:${row.row.runSequence}${queryParam}`);
